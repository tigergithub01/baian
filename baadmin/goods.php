...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $smarty->assign('brand_list',   get_brand_list('CONVERT( brand_name USING gbk ) COLLATE gbk_chinese_ci'));
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        $goods = array(
        	...
        	'gift_start_date' => local_date('Y-m-d'),
        	'gift_end_date'   => local_date('Y-m-d', local_strtotime('+1 month')),
            'goods_weight'  => 0,
            'give_integral' => -1,
            'rank_integral' => -1,
        	'relative_module' => 0,
    ...
    else
    {
        ...
        /* 如果不是买几送几，处理促销日期 */
        if (isset($goods['is_buy_gift']) && $goods['is_buy_gift'] == '0')
        {
        	unset($goods['gift_start_date']);
        	unset($goods['gift_end_date']);
        }
        else
        {
        	$goods['gift_start_date'] = local_date('Y-m-d', $goods['gift_start_date']);
        	$goods['gift_end_date'] = local_date('Y-m-d', $goods['gift_end_date']);
        }
...
//     $is_promote = empty($promote_price) ? 0 : 1;
    ...
    $bonus = isset($_POST['bonus']) ? intval($_POST['bonus']) : '-1';
    ...
    //goods_title
    $goods_title = isset($_POST['goods_title']) ? $_POST['goods_title'] : null;
    
    //pinyin
    $pinyin = isset($_POST['pinyin']) ? $_POST['pinyin'] : null;
    
    //可减少运费金额 added by tiger.guo 20151010
    $reduce_ship_amt = isset($_POST['reduce_ship_amt']) ? $_POST['reduce_ship_amt'] : 0;
    
    //促销商品可减少运费金额 added by tiger.guo 20160702
    $promote_reduce_ship_amt = isset($_POST['promote_reduce_ship_amt']) ? $_POST['promote_reduce_ship_amt'] : 0;
    
    //买几送几,买满赠送件数 added by tiger.guo 20151010
    $is_buy_gift = isset($_POST['is_buy_gift']) ? intval($_POST['is_buy_gift']) : 0;
    $gift_start_date = ($is_buy_gift && !empty($_POST['gift_start_date'])) ? local_strtotime($_POST['gift_start_date']) : 0;
    $gift_end_date = ($is_buy_gift && !empty($_POST['gift_end_date'])) ? local_strtotime($_POST['gift_end_date']) : 0;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,goods_title,reduce_ship_amt,promote_reduce_ship_amt,".
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$goods_title','$reduce_ship_amt','$promote_reduce_ship_amt',".
            		" '$pinyin', '$bonus', '$is_buy_gift', '$gift_start_date', '$gift_end_date', '$purchase_price', '$relative_module', '$promote_limit_num', '$virtual_goods_id')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,goods_title,reduce_ship_amt,promote_reduce_ship_amt,".
                    "pinyin,bonus, is_buy_gift, gift_start_date, gift_end_date, purchase_price, relative_module, promote_limit_num, virtual_goods_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$goods_title','$reduce_ship_amt','$promote_reduce_ship_amt',".
            		" '$pinyin','$bonus', '$is_buy_gift', '$gift_start_date', '$gift_end_date', '$purchase_price', '$relative_module', '$promote_limit_num', '$virtual_goods_id')";
        }
    }
    else
    {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "promote_reduce_ship_amt = '$promote_reduce_ship_amt', " .
                "pinyin = '$pinyin', " .
                "bonus = '$bonus', " .
                "is_buy_gift = '$is_buy_gift', " .
...
//     exit;
    ...
    {
        $link[0] = array('href' => 'goods.php?act=edit&goods_id=' . $goods_id, 'text' => $_LANG['product']);
    }
    if ($code == 'virtual_card')
    {
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    $smarty->assign('action_link2',  array('href' => 'goods.php?act=product_add&goods_id='.$goods_id, 'text' => '添加新货品'));
...
//-- 货品新增、编辑
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'product_add' || $_REQUEST['act'] == 'product_edit'){
	admin_priv('goods_manage');
	
	$is_add = ($_REQUEST['act'] == 'product_add'); // 添加还是编辑的标识
	...
	if ($is_add)
	{
		...
		$sql = "SELECT * FROM " . $ecs->table('products') . " WHERE product_id = '$_REQUEST[product_id]'";
	...
	if ($is_add)
	{
		...
		$product = array(
				...
				'goods_id'    => $goods_id,
				'product_sn'        => '',
				'product_number'    => $_CFG['default_storage'],       
				'product_price'    => 0,
				'is_promote' => 0,
				'promote_start_date' => local_date('Y-m-d'),
				'promote_end_date'   => local_date('Y-m-d', local_strtotime('+1 month')),
	...
	else
	{
		...
		//多仓库库存
		$sql = "SELECT product_store_id, product_id, store_id, product_number FROM " . $ecs->table('products_store') . " WHERE product_id = '".$product['product_id']."'";
		$products_store_list = $db->getAll($sql);
		
		//货品相册
		$sql = "SELECT * FROM " . $ecs->table('products_gallery') . " WHERE product_id = '".$product['product_id']."'";
		$img_list = $db->getAll($sql);
	...
	//为空时插入一条初始记录
	if(empty($products_store_list)){
		$products_store_list = array('0'=>array('store_id'=>0,'product_number'=>0));
	}	
	
	/* 如果不是促销，处理促销日期 */
	if (isset($product['is_promote']) && $product['is_promote'] == '0')
	{
		unset($product['promote_start_date']);
		unset($product['promote_end_date']);
	}
	else
	{
		$product['promote_start_date'] = local_date('Y-m-d', $product['promote_start_date']);
		$product['promote_end_date'] = local_date('Y-m-d', $product['promote_end_date']);
	}
	...
	if (!$is_add){
		foreach ($_attribute as $key => $row) {
			$attr_sql = "SELECT ga.* FROM " . $ecs->table('products_attr') . " AS pa,". $ecs->table('goods_attr'). " AS ga WHERE pa.goods_attr_id = ga.goods_attr_id and ga.attr_id = '".$row['attr_id']."' AND  pa.product_id = '$_REQUEST[product_id]'";
			$goods_attr = $db->getRow($attr_sql);
			$_attribute[$key]['product_attr_value'] = $goods_attr['attr_value'];
		}
	}
...
// 	var_dump($_attribute);
	...
	$smarty->assign('ur_here',      ($is_add?'新增':'编辑').'货品信息');
	$smarty->assign('action_link',  array('href' => 'goods.php?act=product_list&goods_id='.$goods_id, 'text' => $_LANG['18_product_list']));
	$smarty->assign('form_act', $is_add ? 'product_insert' : 'product_update');
...
//-- 货品 保存
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'product_insert' || $_REQUEST['act'] == 'product_update')
{
	admin_priv('goods_manage');
	...
	$promote_price = !empty($_POST['promote_price']) ? floatval($_POST['promote_price'] ) : 0;
	$is_promote = !empty($_POST['is_promote']) ? intval($_POST['is_promote'] ) : 0;
	$promote_start_date = ($is_promote && !empty($_POST['promote_start_date'])) ? local_strtotime($_POST['promote_start_date']) : 0;
	$promote_end_date = ($is_promote && !empty($_POST['promote_end_date'])) ? local_strtotime($_POST['promote_end_date']) : 0;
	$product_sn = isset($_POST['product_sn']) ? $_POST['product_sn'] : '';
	$product_number = empty($_POST['product_number']) ? (empty($_CFG['use_storage']) ? 0 : $_CFG['default_storage']) : trim($_POST['product_number']); //库存
	$product_price = !empty($_POST['product_price']) ? floatval($_POST['product_price'] ) : 0;
	$product_id = isset($_POST['product_id']) ? $_POST['product_id'] : 0;
	$attr  = $_POST['attr'];
	/* 插入还是更新的标识 */
	$is_insert = ($_REQUEST['act'] == 'product_insert');	
	...
	if(empty($attr)){
		sys_msg($_LANG['sys']['wrong'] . '请选择属性值', 1, array(), false);
	}
	...
	if (empty($goods))
	{
		...
		foreach($attr as $attr_key => $attr_value)
		{
			...
			$value_price_list[$attr_key] = $attr_value . chr(9) . ''; //$key，当前

			$id_list[$attr_key] = $attr_key;
		}
		$goods_attr_id = handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list);

		/* 是否为重复规格的货品 */
		$goods_attr = sort_goods_attr_id_array($goods_attr_id);
		$goods_attr = implode('|', $goods_attr['sort']);
		if (check_goods_attr_exist($goods_attr, $goods_id,$product_id))
		{
// 			continue;
		...
		if (!empty($product_sn))
		{
			...
			if (check_goods_sn_exist($product_sn))
			{
		...
		if ($is_insert){
			/* 插入货品表 */
			$sql = "INSERT INTO " . $GLOBALS['ecs']->table('products') . " (goods_id, goods_attr, product_sn, product_number,product_price,is_default, ".
					" is_promote , promote_price ,  promote_start_date, promote_end_date) ".
				 "	VALUES ('$goods_id', '$goods_attr', '$product_sn', '$product_number','$product_price','$is_default', " . 
				 " '$is_promote' , '$promote_price' , '$promote_start_date','$promote_end_date') ";
		}else{
			/* 更新货品表 */
			$sql = "UPDATE " . $ecs->table('products') . " SET " .
					"goods_id = '$goods_id', " .
		...
		//获取插入后的货品编号
		$product_id = $is_insert?$GLOBALS['db']->insert_id():$_REQUEST['product_id'];
		
		//插入货品属性表
		$goods_attrs = explode("|", $goods_attr);
		$sql = "DELETE FROM ". $GLOBALS['ecs']->table('products_attr') ." WHERE product_id = '".$product_id."'";
		$GLOBALS['db']->query($sql);
		
		foreach ($goods_attrs as $goods_attr_id) {
			if(!empty($goods_attr_id)){
				$sql = "INSERT INTO ". $GLOBALS['ecs']->table('products_attr') ."(product_id, goods_attr_id) VALUES ('".$product_id."','$goods_attr_id')";
				$GLOBALS['db']->query($sql);
			}
		}
		...
		$sql = "DELETE FROM ". $GLOBALS['ecs']->table('products_store') ." WHERE product_id = '".$product_id."'";
		$GLOBALS['db']->query($sql);
		...
		/* 修改商品表库存 */
		$product_total_number = product_number_count($goods_id);
		if (update_goods($goods_id, 'goods_number', $product_total_number))
		{
			//记录日志
			admin_log($goods_id, 'update', 'goods');
		}
		...
		/* 处理相册图片 */
		handle_product_gallery_image($goods_id, $product_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file']);
		...
		if (isset($_POST['old_img_desc']))
		{
			foreach ($_POST['old_img_desc'] AS $img_id => $img_desc)
			{
				$sql = "UPDATE " . $ecs->table('products_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
				$db->query($sql);
			}
		}
	...
	if ($is_insert)
	{
		$link[] = array('href' => 'goods.php?act=product_add&goods_id=' . $goods_id, 'text' => '继续添加新货品');
...
elseif ($_REQUEST['act'] == 'product_remove')
{
    ...
    /* 已经加入购物车中的货品不能删除 */
    $sql = "select count(1) FROM " . $ecs->table('cart') . " WHERE product_id = '$product_id'";
    $count = $db->getOne($sql);
    if($count>=1){
    	make_json_error("已经加入购物车中的货品不能删除！");
    }
    
    
    /* 订单中已经存在的货品不能删除 */
    $sql = "select count(1) FROM " . $ecs->table('order_goods') . " WHERE product_id = '$product_id'";
    $count = $db->getOne($sql);
    if($count>=1){
    	make_json_error("订单中已经存在的货品不能删除！");
    }
...
elseif ($_REQUEST['act'] == 'edit_product_price')
{
	...
	$product_id       = intval($_POST['id']);
	$product_price       = intval($_POST['val']);
...
//-- 修改货品仓库
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_store_id')
{
	check_authz_json('goods_manage');
	...
	if(empty($store_id)){
		make_json_error($_LANG['sys']['wrong'] . '仓库不能为空');
	}
		
	/* 修改货品仓库 */
	$sql = "UPDATE " . $ecs->table('products') . " SET store_id = '$store_id' WHERE product_id = '$product_id'";
	$result = $db->query($sql);
	if ($result)
	{
		clear_cache_files();
...
//-- 货品相册
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'product_gallery_list')
{
	admin_priv('goods_manage');
	...
	if (empty($product_id)){
		sys_msg($_LANG['sys']['wrong'] . '货品不存在1', 1, array(), true);
	}
	...
	if (!isset($product)){
	 ...
	 /* 图片列表 */
     $sql = "SELECT * FROM " . $ecs->table('products_gallery') . " WHERE product_id = '$product_id'";
     $img_list = $db->getAll($sql);
	...
	$smarty->display('product_gallery.htm');
}
...
elseif ($_REQUEST['act'] == 'product_gallery_update')
{
	...
	/* 处理相册图片 */
	handle_product_gallery_image($product['goods_id'],$product_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file']);
...
//-- 删除图片
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_product_image')
{
	check_authz_json('goods_manage');

	$img_id = empty($_REQUEST['img_id']) ? 0 : intval($_REQUEST['img_id']);

	remove_product_image($img_id);
...
//-- 修改上架状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_on_product_default')
{
	check_authz_json('goods_manage');

	$product_id       = intval($_POST['id']);
	$is_default        = intval($_POST['val']);

	$exc_product = new exchange($ecs->table('products'), $db, 'product_id', 'product_sn');
	if ($exc_product->edit("is_default = '$is_default'", $product_id))
	{
		clear_cache_files();
		make_json_result($is_default);
	}
}
