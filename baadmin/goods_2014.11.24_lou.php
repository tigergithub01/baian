...
//	die('here');
	...
	/* pgge关联版式 */
	$sql = 'SELECT module_name,module_id FROM ' .$GLOBALS['ecs']->table('relative_module'). ' ';
	$relative_module = $db->getAll($sql);
	/*$relative_module = array();
	foreach($relative_module as $key=>$val)
	{
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
		...
		/*add by wanggz 设置某个商品在在某些地区可以包邮，某些地区不能*/  
		/* 获得该区域下的所有地区 */
		$regions = array();
		$sql = "SELECT a.region_id, r.region_name ".
            "FROM ".$ecs->table('goods_free_shipping_area')." AS a, ".$ecs->table('region'). " AS r ".
            "WHERE r.region_id=a.region_id AND a.goods_id='$goods[goods_id]'";
		$res = $db->query($sql);
		...
		/*add by wanggz 设置某个商品在在某些地区可以包邮，某些地区不能end*/  
        ...
        {
            $goods['qr_code'] = get_image_path($_REQUEST['goods_id'], $goods['qr_code']);
    ...
    /* 模板赋值 */
    $smarty->assign('action_list', $_SESSION['action_list']);
    $smarty->assign('code',    $code);
    ...
    /*add by wanggz设置某个商品在在某些地区可以包邮，某些地区不能*/
	$smarty->assign('countries',        get_regions());
    $smarty->assign('default_country',  $_CFG['shop_country']);
	/*add by wanggz设置某个商品在在某些地区可以包邮，某些地区不能end*/
...
//	var_dump($GLOBALS);die();
    ...
    if (isset($_FILES['qr_code']['error'])) // php 4.2 版本才支持 error
    {
        // 最大上传文件大小
    ...
    $old_original_img = '';  // 初始化原始图片旧图
	 ...
	 // 是否上传二维码图片
    if (isset($_FILES['qr_code']) && $_FILES['qr_code']['tmp_name'] != '' &&
            isset($_FILES['qr_code']['tmp_name']) &&$_FILES['qr_code']['tmp_name'] != 'none')
    {
        // 上传了，直接使用，原始大小
        $qr_code = $image->upload_image($_FILES['qr_code']);
        if ($qr_code === false)
        {
    ...
    /*微信购买减少金额*/
    $wxbuy_cut = !empty($_POST['wxbuy_cut']) ? $_POST['wxbuy_cut'] : 0;
    
    $promote_price = !empty($_POST['promote_price']) ? floatval($_POST['promote_price'] ) : 0;
    $is_promote = empty($promote_price) ? 0 : 1;
	//$momsuit_price = !empty($_POST['momsuit_price']) ? floatval($_POST['momsuit_price'] ) : 0;
    //$is_momsuit = empty($momsuit_price) ? 0 : 1;
	...
	$Freight_assessment = isset($_POST['Freight_assessment']) ? $_POST['Freight_assessment'] : 0; //运费评估
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            ...
            /*$sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, purchase_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, qr_code, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, is_momsuit, momsuit_price)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$purchase_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$qr_code', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$is_momsuit', '$momsuit_price')";
			*/

			$sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, purchase_price, wxbuy_cut, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, qr_code, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, bonus, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,goods_title, ".
					"suppliers_id,Freight_assessment)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$purchase_price', '$wxbuy_cut', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$qr_code', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$_POST[bonus]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$_POST[goods_title]', ".
					"'$Freight_assessment')";
        }
        else
        {
            ...
            /*$sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, is_momsuit, momsuit_price)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$purchase_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$qr_code', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$is_momsuit', '$momsuit_price')";
			*/

			$sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, purchase_price, wxbuy_cut, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, qr_code, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, bonus, give_integral, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,goods_title, ".
					"rank_integral,Freight_assessment)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$purchase_price', '$wxbuy_cut', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$qr_code', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$_POST[bonus]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$_POST[goods_title]', ".
					"'$Freight_assessment')";
        }
    }
    else
    {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img, original_img, qr_code " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);

        if ($proc_thumb && $goods_img && $row['qr_code'] && !goods_parse_url($row['qr_code']))
        {
            @unlink(ROOT_PATH . $row['qr_code']);
        }
        ...
        /*$sql = "UPDATE " . $ecs->table('goods') . " SET " .
				...
				"is_momsuit = '$is_momsuit'," .
				"momsuit_price = '$momsuit_price', " .
		...
		$sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "suppliers_id = '$suppliers_id', " .
                "promote_end_date = '$promote_end_date', ".
				"Freight_assessment = '$Freight_assessment', ";
	...
	$sql = "UPDATE " .$ecs->table('goods'). " SET buy_number_activity = '$_POST[buy_number_activity]' WHERE goods_id = '$goods_id' LIMIT 1";
	$db->query($sql);
...
/*add by wanggz设置某个商品在在某些地区可以包邮，某些地区不能*/  
        ...
        /* 清除原有的城市和地区 */
        $db->query("DELETE FROM ".$ecs->table("goods_free_shipping_area")." WHERE goods_id='$goods_id'");

        /* 添加选定的城市和地区 */
        foreach ($selected_regions AS $key => $val)
        {
            $sql = "INSERT INTO ".$ecs->table('goods_free_shipping_area')." (goods_id, region_id) VALUES ('$goods_id', '$val')";
            $db->query($sql);
        }
		/*add by wanggz设置某个商品在在某些地区可以包邮，某些地区不能end*/  
            ...
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
                ...
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
						...
						$goods_attr_list[$attr_id][$attr_value]['attr_images'] = $attr_images;  //代码增加  By www.68ecshop.com
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
				...
				if( $info['sign'] == 'insert' || $info['sign'] == 'update' )
				{
					...
					{
						$attr_images_dir = 'color/'.get_attachment_www_68ecshop_com($goods_id);
				...
				{
					$attr_color_images_dir = ROOT_PATH.DATA_DIR.'/color/'.get_attachment_www_68ecshop_com($goods_id)."/0-0-".$info[goods_attr_id].".jpg";
	...
	$qr_code = reformat_image_name('qr_code', $goods_id, $qr_code,  'qr_code');//新的图片
    $goods_thumb = reformat_image_name('goods_thumb', $goods_id, $goods_thumb, 'thumb');


	if ($qr_code !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET qr_code = '$qr_code' WHERE goods_id='$goods_id'");
    }
    ...
    {
        $link[0] = array('href' => 'goods.php?act=product_storeroom&goods_id=' . $goods_id, 'text' => $_LANG['product']);  //这行代码 有修改 By www.ecshop120.com 多城市多仓库
    }
    if ($code == 'virtual_card')
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
		...
		/* pgge关联版式 */
        elseif ($_POST['type'] == 'relative_module')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
            update_goods($goods_id, 'relative_module', $_POST['target_module_id']);
        }
		 elseif ($_POST['type'] == 'cancel_relative_module')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
            update_goods($goods_id, 'relative_module', 0);
        }
		/* pgge关联版式 end*/
		...
		elseif ($_POST['type'] == 'other_cat')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
			foreach($_POST['checkboxes'] as $goods_id)
			{
				$k=0;
				$sql  = "replace ". $ecs->table('goods_cat') ." values" ;
				if($_POST['other_cat_1'])
				{
					$sql .= $k==0 ? "" : ",";
					$sql .="('".$goods_id."','". $_POST['other_cat_1'] ."')";
					$k++;
				}
				if($_POST['other_cat_2'])
				{
					$sql .= $k==0 ? "" : ",";
					$sql .="('".$goods_id."','". $_POST['other_cat_2'] ."')";
					$k++;
				}
				if($_POST['other_cat_3'])
				{
					$sql .= $k==0 ? "" : ",";
					$sql .="('".$goods_id."','". $_POST['other_cat_3'] ."')";
					$k++;
				}
				if($_POST['other_cat_4'])
				{
					$sql .= $k==0 ? "" : ",";
					$sql .="('".$goods_id."','". $_POST['other_cat_4'] ."')";
					$k++;
				}
				if($_POST['other_cat_5'])
				{
					$sql .= $k==0 ? "" : ",";
					$sql .="('".$goods_id."','". $_POST['other_cat_5'] ."')";
					$k++;
				}
				$db->query($sql);
			}
...
//    var_dump($_POST);
//    var_dump($_GET);
//    var_dump("goods_number = '$goods_num', last_update=" .gmtime());
//    var_dump($goods_id);die();

//<pre>array(3) {
//  ["act"]=>
//  string(17) "edit_goods_number"
//  ["val"]=>
//  string(2) "23"
//  ["id"]=>
//  string(4) "1883"
...
/* 代码增加_start  By ecshop120.com 多城市多仓库*/
elseif ($_REQUEST['act'] == 'product_storeroom')
{
	$goods_id   =  $_REQUEST['goods_id'];
      /* 取出商品信息 */
    $sql_wwwECSHOP120com = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $ecs->table('goods') . " WHERE goods_id = '$goods_id'";
    $goods_wwwECSHOP120com = $db->getRow($sql_wwwECSHOP120com);
	$smarty->assign('goods_id', $goods_id);
	$smarty->assign('goods_name', sprintf($_LANG['products_title'], $goods_wwwECSHOP120com['goods_name']));
    $smarty->assign('goods_sn', sprintf($_LANG['products_title_2'], $goods_wwwECSHOP120com['goods_sn']));

	$sql="select * from ".$ecs->table('products_storeroom')." where goods_id='$goods_id'";
	$res_ps=$db->query($sql);
	$result_old=array();
	while ($row_ps=$db->fetchRow($res_ps))
	{
		$key_old = $row_ps['goods_attr'];
		$key_old .= $key_old ? "|" : "";
		$key_old .= $row_ps['store_id'];
		$result_old[$key_old] =  $row_ps['store_number'];
	}

    require_once(ROOT_PATH . '/' . ADMIN_PATH . '/includes/lib_pailie.php');
	$auto_attr_temp = get_goods_specifications_list($_REQUEST['goods_id']);

    $sql_store_wwwECSHOP120com = "select store_id, store_name from ".$ecs->table("goods_storeroom")." order by sort_order ";
	$res_store_wwwECSHOP120com = $db->query($sql_store_wwwECSHOP120com);
	while($row_store_wwwECSHOP120com = $db->fetchRow($res_store_wwwECSHOP120com))
	{
		$auto_attr_temp[]=array('goods_attr_id'=>$row_store_wwwECSHOP120com['store_id'], 'attr_value'=>$row_store_wwwECSHOP120com['store_name'],  'attr_id'=>'999','attr_name'=>'仓库');
	}


	//echo '<pre>';
	//print_r($auto_attr_temp);
	//echo '</pre>';

	$Result=get_pailie_zuhe($auto_attr_temp);
	...
	foreach($Result as $key_result=> $value_result)
	{
		...
		$Result[$key_result]['value'] = $result_value;
		$key_old = $result_value;
		$key_old .= $key_old ? "|" : "";
		$key_old .= $value_result['仓库']['goods_attr_id'];
		$Result[$key_result]['number'] =  $result_old[$key_old];
	}
	...
	$smarty->display('product_storeroom.htm');

}

/* 保存货品库房库存  */
elseif ($_REQUEST['act'] == 'product_storeroom_exe')
{
	$goods_id   =  $_REQUEST['goods_id'];
	$store_id_arr = $_REQUEST['store_id'];
	$store_number_arr = $_REQUEST['store_number'];
//	echo"<pre>";var_dump($goods_id);var_dump($store_id_arr);var_dump($store_number_arr);var_dump($_REQUEST['goods_attr']);//die();
...
//}
	...
	foreach($_REQUEST['goods_attr'] as $key_store => $goods_attr)
	{

		$sql="replace into ". $ecs->table('products_storeroom') ."(goods_id, goods_attr, store_id, store_number ) values('$goods_id', '$goods_attr', '".$store_id_arr[$key_store]."', '".$store_number_arr[$key_store]."')";
		$db->query($sql);
	}

	$sql_product_wwwecshop120com=" SELECT goods_attr, sum( store_number ) AS product_number FROM ". $ecs->table('products_storeroom'). " WHERE goods_id ='$goods_id' GROUP BY goods_attr";
	$res_product_wwwecshop120com = $db->query($sql_product_wwwecshop120com);
//	echo "<pre>";
//	var_dump($sql_product_wwwecshop120com);
//	var_dump($res_product_wwwecshop120com);die();
	while ($row_product_wwwecshop120com = $db->fetchRow($res_product_wwwecshop120com))
	{
//		echo "<pre>";
//		var_dump($row_product_wwwecshop120com);
		$sql_product_qq2211707 = "select product_id from ". $ecs->table('products') ." where goods_id='$goods_id' and  goods_attr='$row_product_wwwecshop120com[goods_attr]'  " ;
		$row_product_qq2211707 = $db->getOne($sql_product_qq2211707);
		if($row_product_qq2211707)
		{
			$sql_qq2211707="update ".$ecs->table('products')." set product_number='".$row_product_wwwecshop120com['product_number']."' where product_id='$row_product_qq2211707' ";
		}
		else
		{
			$sql_qq2211707="insert into ".$ecs->table('products')."(goods_id, goods_attr, product_sn, product_number ) values('$goods_id', '".$row_product_wwwecshop120com['goods_attr']."', '', '".$row_product_wwwecshop120com['product_number']."' )";
		}
		$db->query($sql_qq2211707);
	}
	
//	die();

	// 更新一下商品库存
    $sql = "SELECT SUM(store_number)
            FROM " . $GLOBALS['ecs']->table('products_storeroom') . "
            WHERE goods_id = '$goods_id'";
    $nums = $GLOBALS['db']->getOne($sql);
    $nums = empty($nums) ? 0 : $nums;
    	
	update_goods($goods_id,'goods_number',$nums);
	
	/* 清除缓存 */
    clear_cache_files();

    $link[0]['text'] = '返回库房货品列表页';
    $link[0]['href'] = 'goods.php?act=list&uselastfilter=1';

    sys_msg("按库房添加货品成功", 0, $link);

}


/* 代码增加_end  By ecshop120.com */
