...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if (empty($order))
    {
		...
		/*--pgge显示下单人手机号以有所在地区插件修改过代码--*/
	if($order['mobile_area']=='手机号码格式错误！' || $order['mobile_area']=='没有此号码记录' || empty($order['mobile_area']))
	{
		if(!empty($order['mobile']))
		{
			$order['mobile_area'] = get_mobile_api_area($order['mobile']);
			$sql = "update " . $ecs->table('order_info') . " set mobile_area='".$order['mobile_area']."' WHERE order_id = '$order[order_id]'";
			$db->query($sql);
		}
	}
    ...
    $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, ''), '  ',IFNULL(w.region_name, '')) AS region " .
            "FROM " . $ecs->table('order_info') . " AS o " .
                "LEFT JOIN " . $ecs->table('region') . " AS c ON o.country = c.region_id " .
                "LEFT JOIN " . $ecs->table('region') . " AS p ON o.province = p.region_id " .
                "LEFT JOIN " . $ecs->table('region') . " AS t ON o.city = t.region_id " .
                "LEFT JOIN " . $ecs->table('region') . " AS d ON o.district = d.region_id " .
                "LEFT JOIN " . $ecs->table('region') . " AS w ON o.town = w.region_id " .
    ...
    while ($row = $db->fetchRow($res))
    {
		...
		elseif ($row['suit_id'] > 0)
		{
			$row['formated_subtotal']       = '准妈妈套装商品';
			$row['formated_goods_price']    = '准妈妈套装商品';
		}
...
/*--pgge显示下单人IP插件修改过代码--*/
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,is_deposit, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,ip ".
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ". $where .
                " ORDER BY $filter[sort_by] $filter[sort_order] ".
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
/*--pgge显示下单人IP插件修改过代码 end--*/
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['is_deposit']          =  $value['is_deposit'];
