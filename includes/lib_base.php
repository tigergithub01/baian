...
//创建cookie通过客户端的ip
    setcookie("real_ipd", $realip, time()+18000, "/");
    return $realip;
}
...
/**
 * 获取ip地址信息 - 网络版本 2015-10-08 tiger.guo
 * @param   string      $ip        字符串
 * @return  string
 */
...
/*--pgge显示下单人IP以有所在地区插件修改过代码--*/
function get_ip_api_area($ip)
{
	$url = "http://ip.taobao.com/service/getIpInfo.php?ip=$ip"; 
	if(function_exists('file_get_contents'))
	{
	   $areaInfo = @file_get_contents($url);
	}
	else
	{
		$ch = curl_init();
		...
		$areaInfo = curl_exec($ch);
		curl_close($ch);
	}
	
    $areaInfo= json_decode($areaInfo);
	
	$country_name = $areaInfo->data->country;
	$province_name = $areaInfo->data->region;
	$city_name = $areaInfo->data->city;
    $isp = $areaInfo->data->isp;
	
	$areaInfo = $country_name.$province_name.$city_name.' '.$isp;
	return $areaInfo;
} 
/*--pgge显示下单人IP以有所在地区插件修改过代码end--*/


/*--pgge显示下单人手机号以有所在地区插件修改过代码--*/
function get_mobile_api_area($mobile)
{
