...
if(($ua == '' || preg_match($uachar, $ua)) && !strpos(strtolower($_SERVER['REQUEST_URI']),'wap')){
	class alipay
	{
		...
		function get_code($order, $payment)
		{
			...
			$button = '<div style="text-align:center"><input type="button" class="s-btn1" onclick="window.open(\''.$gateway.$param. '&sign='.md5($sign).'\')" value="' .$GLOBALS['_LANG']['pay_button']. '" /></div>';
			return $button;
		}
...
//        }
        ...
        $button = '<div style="text-align:center"><input type="button" class="s-btn1" onclick="window.open(\'https://www.alipay.com/cooperate/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5\')" value="' .$GLOBALS['_LANG']['pay_button']. '" /></div>';
