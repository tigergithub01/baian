...
$sql = "select goods_id from ".$ecs->table('goods')." where goods_name not like '%自行车%'";

$goods_id_arr = $db->getCol($sql);
//echo "<pre>";
//var_dump($goods_id_arr);die();
foreach($goods_id_arr as $key=>$val){
	$sql="insert into ".$ecs->table('products_storeroom')."(goods_id, goods_attr, store_id, store_number ) values('$val', '', '12', '100' )";
	$db->query($sql);
}
die('ok');
