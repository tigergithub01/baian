...
* 获得首页每天显示抢购商品
*/
function get_promotegoods( $num )
{
		$arr = array( );

		//$sql = "select goods_id,goods_name,ROUND(market_price,1) shop_price,ROUND(promote_price,1) promote_price,goods_thumb,promote_start_date,promote_end_date from ".$GLOBALS['ecs']->table( "goods" )." where is_delete=0 and is_promote=1 AND DATEDIFF(FROM_UNIXTIME(promote_start_date),NOW())<=0 AND DATEDIFF(FROM_UNIXTIME(promote_end_date),NOW())>0 limit ".$num;
        
        $time = gmtime();
        $sql = "select goods_id,goods_name,ROUND(market_price,1) shop_price,ROUND(promote_price,1) promote_price,goods_thumb,promote_start_date,promote_end_date from ".$GLOBALS['ecs']->table( "goods" )." where is_on_sale = 1 AND is_delete=0 and is_promote=1 AND promote_start_date <= '$time' AND promote_end_date >= '$time' limit ".$num;
		...
		foreach ( $res as $idx => $row )
		{
				...
				$arr[$row['goods_id']]['shop_price']    = price_format( $row['shop_price'] );//floatval($row['shop_price']);
				$arr[$row['goods_id']]['promote_price'] = price_format($row['promote_price']);
				$arr[$row['goods_id']]['goods_thumb']   = $row['goods_thumb'];
				$arr[$row['goods_id']]['goods_url']   = build_uri( "goods", array(
						"gid" => $row['goods_id']
				), $row['goods_name'] );
		}
...
获得最新指定个数的套装
*/
function get_momsuit( $num )
{
		$arr = array( );
		$sql = "select Suit_Name,Suit_img,Suit_ID from ".$GLOBALS['ecs']->table( "momsuit" )." where DATEDIFF(FROM_UNIXTIME(start_time),NOW())<=0 AND DATEDIFF(FROM_UNIXTIME(end_time),NOW())>0 order by Suit_ID desc limit ".$num;
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $idx => $row )
		{
				$arr[$row['Suit_ID']]['Suit_ID']      = $row['Suit_ID'];
				$arr[$row['Suit_ID']]['Suit_Name']    = $row['Suit_Name'];
				$arr[$row['Suit_ID']]['Suit_img']     = $row['Suit_img'];
		}
		return $arr;
}
...
/**
 */
function get_my_articles_two( $typeid, $count=1 )
{
        
    $sql = 'SELECT article_id, title, author, add_time, file_url, open_type, content' .
           ' FROM ' .$GLOBALS['ecs']->table('article') .
           ' WHERE is_open = 1 AND cat_id=' . $typeid .
           ' ORDER BY article_type DESC, article_id DESC limit '.$count;
    ...
    foreach ( $res as $idx => $row )
        ...
        $arr[$idx]['url']         = $row['open_type'] != 1 ? build_uri('article', array('aid'=>$article_id), $row['title']) : trim($row['file_url']);
...
function get_my_articles( $ids, $num, $tlen, $clen )
{
    $arr = array();
    
    $sql = "SELECT article_id, title, author, add_time, file_url, open_type, content" .
           " FROM " .$GLOBALS['ecs']->table('article') .
           " WHERE is_open = 1 AND cat_id = ".$ids."" .
           " ORDER BY article_id DESC limit " .$num;
    ...
    foreach ( $res as $idx => $row )
        ...
        $title = $row['title'];
				$arr[$idx]['title']         = $title;
				$arr[$idx]['short_title']   = strlen($title) > $tlen ? msubstr($title,0,$tlen,'utf-8',false) : $title;
				$arr[$idx]['author']        = empty($row['author']) || $row['author'] == '_SHOPHELP' ? $GLOBALS['_CFG']['shop_name'] : $row['author'];
				$arr[$idx]['url']           = $row['open_type'] != 1 ? build_uri('article', array('aid'=>$row['article_id']), $row['title']) : trim($row['file_url']);
				$arr[$idx]['add_time']      = date($GLOBALS['_CFG']['date_format'], $row['add_time']);
				$arr[$idx]['short_time']    = date('n-d', $row['add_time']);
				$arr[$idx]['content']       = $row['content'];
        
        $c = strip_tags($row['content']);
        $arr[$idx]['short_content'] = strlen($c) > $clen ? msubstr($c,0,$clen,'utf-8',false) : $c ;
		}
...
function get_qq_no()
{
    ...
    $sql = "SELECT value FROM " .$GLOBALS['ecs']->table('shop_config') . " WHERE code = 'qq'";
