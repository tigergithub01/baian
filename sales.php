 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: sales.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
$_REQUEST['act'] = !empty($_REQUEST['act']) ? trim($_REQUEST['act']) : '';
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $db->autoReplace($ecs->table('keywords'), array('date' => local_date('Y-m-d'),
                'salesengine' => 'ecshop', 'keyword' => addslashes(str_replace('%', '', $val)), 'count' => 1), array('count' => 1));
        }
        $keywords .= ')';
    ...
    $display  = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display'])  : (isset($_SESSION['display_sales']) ? $_SESSION['display_sales'] : $default_display_type);

    $_SESSION['display_sales'] = $display;
    ...
    $smarty->assign('sales_keywords',   stripslashes(htmlspecialchars_decode($_REQUEST['keywords'])));
    ...
    /* 分页 */
    $url_format = "sales.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" . $_REQUEST['brand']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
    if (!empty($intromode))
    {
    ...
    $pager['sales'] = array_merge($pager['sales'], $attr_arg);

    $pager = get_pager('sales.php', $pager['sales'], $count, $page, $size);
