...
function getStockCallback(result) {
    if (result.stock && (result.stock.code == 3 || result.stock.code == 4) && result.stock.level > 1) {
        ...
        var JYXL_MYF = parseInt($('#JYXL_MYF').val()); 
        if (result.length > 0) {
            var ftat = parseInt(result[0].FTAT);
            //alert(JYXL_BL)
            for (var i = 0; i < parseInt(ftat / JYXL_BL); i++) {
                jyHtml += "<img src=\"themes/baian/images/ba_jyxl_l.gif\" style=\"width:20px; height:20px;\" title=\"笑脸\">";
            }

            for (var j = 0; j < parseInt(ftat % JYXL_BL); j++) {
                jyHtml += "<img src=\"themes/baian/images/ba_jyxl_j.gif\" style=\"width:15px; height:15px;\" title=\"脚印\">";
            }
            
            if(jyHtml){
                jyHtml = '运费评估：' + jyHtml;
                jyHtml += '&nbsp;&nbsp;（'+
                		  '<span style="color:black;font-size:13px;font-weight:bold;">注</span>：'+
						  //JYXL_BL+'<img src=\"themes/baian/images/ba_jyxl_j.gif\" style=\"width:15px; height:15px;\" title=\"脚印\">=1<img src=\"themes/baian/images/ba_jyxl_l.gif\" style=\"width:20px; height:20px;\" title=\"笑脸\">'+
						  '全场商品累积满'+parseInt(JYXL_MYF / JYXL_BL)+'<img src=\"themes/baian/images/ba_jyxl_l.gif\" style=\"width:20px; height:20px;\" title=\"笑脸\">免运费！）</span>';
            }

            $("#jyxl").html(jyHtml);
        }
        else {
        	//$("#jyxl").css('display','none');
            $("#jyxl").html(jyHtml);
        }
...
function Request(strName) {
    ...
    if(strHref.indexOf('.html')!=-1){
    	//alert(strHref.indexOf('.html'));
    	//alert(strHref.indexOf('goods-'));
    	//alert(strHref.substring(strHref.indexOf('goods-')+6,strHref.indexOf('.html')));
    	return strHref.substring(strHref.indexOf('goods-')+6,strHref.indexOf('.html'));
    }
...
function GetStockInfoOrNextAreas(skuKey, provinceId, cityId, areaId, townId, sortId, curLevel) {
    try {
        ...
        if (curLevel == requestLevel) {
            ...
            if (skuKey && provinceId != 84 && warestatus == 1 || 1) {
                //这里应该是加载送货信息的了
                var pid = Request("id");
...
//                        + townId + "&sortid1=" + pageConfig.product.cat[0] + "&sortid2=" + pageConfig.product.cat[1] + "&sortid3=" + pageConfig.product.cat[2]);
				...
				//alert("/gds.php?callback=getStockCallback&provinceid=" + provinceId + "&cityid=" + cityId + "&areaid=" + areaId + "&townid=" + townId + "&gid=" + pid);
...
function getAreaListcallback2(result) {
	...
	//alert('here!');
    CurrentAreaInfoInit();	//currentAreaInfo
    initrequestLevel();	    //requestLevel = 4;	isAreaProduct = false;
