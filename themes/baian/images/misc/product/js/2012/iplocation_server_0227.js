...
(function () { $.getJSONP("http://easygo.jd.com/switch/get", getEasyBuyState); })();
...
function getLocSkuDate() {
    $.getJSONP("http://rms.shop.360buy.com/json/locWare/getLocWareBySkuId.action?callback=getLocSkuDateCallBack&skuId=" + pageConfig.product.skuid);
}
...
function setGiftTips(isGift, areaText) {
    if (checkApecialAttr("isSupportCard")) {
        ...
        var giftTipsHtml = (areaText || '北京地区') + '支持礼品包装贺卡' + '<a href="http://cart.gift.jd.com/cart/cart.html" target="_blank">&nbsp;&nbsp;详情 <s class="s-arrow">&gt;&gt;</s></a>';
        if (isGift) {
...
function SetNotifyByNoneStock(stockstatus, ext) {
    ...
    if (stockstatus != 34 && stockstatus != 0 && warestatus == 1) {
        ...
        if (ext && ext.indexOf("YuShou") > -1) {
            ...
            if (!window.Qiang) {
                $.ajax({ url: "http://misc.360buyimg.com/product/js/2012/qiang.js?t=20130716", dataType: 'script', cache: true, type: "get", success: function () { if (Qiang && Qiang.init) Qiang.init(G.sku, G.key); } });
            }
            else {
                if (Qiang && Qiang.init) {
...
function getChildAreaHtml(fid, level) {
    ...
    if (idName) {
        ...
        if (level == 2 && provinceCityJson[fid + ""]) {
            ...
            //$.getJSONP("http://d.360buy.com/area/get?fid=" + fid + "&callback=getAreaListcallback");//原版的
...
//    $('<div style="padding-bottom:5px;"><iframe id="xjd_iframe" scrolling="no" frameborder="0" style="width:211px;height:261px;" marginheight="0" marginwidth="0" src="http://x.jd.com/ad?ad_id=57&tag=' + pageConfig.product.cat[2] + '"></iframe></div>').appendTo(".left:first");
//}