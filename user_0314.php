...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'deposit_ship', 'deposit_list', 'deposit_detail', 'address_list', 'collection_list',
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
        {
            ...
            {
                show_message($_LANG['invalid_captcha'].' A', $_LANG['sign_up'], 'user.php?act=register', 'error');
            }
            ...
            {
                show_message($_LANG['invalid_captcha'].' B', $_LANG['sign_up'], 'user.php?act=register', 'error');
            }
        }
...
elseif ($action == 'act_edit_profile')
{
	...
	$baby_birthday = trim($_POST['baby_birthdayYear']) .'-'. trim($_POST['baby_birthdayMonth']) .'-'.
    trim($_POST['baby_birthdayDay']);
...
elseif ($action == 'deposit_list')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('deposit'). " WHERE user_id = '$user_id'");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $deposits = get_user_deposits($user_id, $pager['size'], $pager['start']);
    //$merge  = get_user_merge($user_id);

	//$smarty->assign('lang',  $_LANG);//获得语音文字
    //$smarty->assign('action',  $action);
    $smarty->assign('pager',  $pager);
...
elseif ($action == 'deposit_detail')
{
    ...
    /* 寄存详情 */
    $deposit = get_deposit_detail($deposit_id, $user_id);
    ...
    /* 寄存商品 */
    $goods_list = deposit_goods($deposit_id);
    ...
    $smarty->assign('deposit_id', $deposit_id);
...
elseif ($action == 'deposit_ship')
{
	...
	//身份证
	$idcard = $_POST['idcard'];
	$user_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('users'). " WHERE user_id = ".$user_id." AND ID_cards='".$idcard."'");
	if ($user_count <= 0)
	{
		show_message('输入的身份证号码不符。');
	}
	else
	{
		$deposit_id = $_POST['depositid'];//寄存ID
		$order_id = $_POST['orderid'];//寄存所属订单ID
		$order = array();
		...
		{
			$order['shipping_id']     = $row['shipping_id']; //用户选择的配送方式id
			$order['shipping_name']   = $row['shipping_name']; //用户选择的配送方式的名称
			$order['pay_id']          = $row['pay_id'];  //用户选择的支付方式的id
			$order['pay_name']        = $row['pay_name']; //用户选择的支付方式的名称
			$order['pack_id']         = $row['pack_id']; //用户选择的支付方式的名称
			$order['surplus']         = $row['surplus'];
			$order['integral']        = $row['integral'];
			$order['bonus_id']        = $row['bonus_id'];
			$order['need_inv']        = $row['need_inv'];
			$order['how_oos']         = $row['how_oos'];  //缺货处理方式，等待所有商品备齐后再发； 取消订单；与店主协商
			$order['how_surplus']     = $row['how_surplus'];  //根据字段猜测应该是余额处理方式，程序未作这部分实现
			$order['need_insure']     = $row['need_insure'];
			$order['agency_id']       = $row['agency_id'];
			...
			$order['pack_name']       = $row['pack_name']; //包装名称
		}

		$order['user_id']         = $_SESSION['user_id'];
		$order['add_time']        = gmtime();
		$order['order_status']    = 1; //订单状态。0，未确认；1，已确认；2，已取消；3，无效；4，退货；
		$order['shipping_status'] = 0; //商品配送情况，0，未发货；1，已发货；2，已收货；3，备货中
		$order['pay_status']      = 2; //支付状态；0，未付款；1，付款中；2，已付款

		$order['card_id']         = 0;  //贺卡ID
		$order['card_message']    = ""; //贺卡内容
		$order['card_name']       = ""; //贺卡的名称
		$order['inv_type']        = ""; //发票类型
		$order['inv_payee']       = ""; //发票抬头
		$order['inv_content']     = ""; //发票内容
		...
		$order['consignee']    = $_POST['consignee']; //收货人姓名
		$order['address']      = $_POST['address']; //收货人的详细地址
		$order['zipcode']      = $_POST['zipcode']; //'收货人的邮编
		$order['tel']          = $_POST['tel']; //收货人的电话
		$order['mobile']       = $_POST['mobile']; //收货人手机
		$order['email']        = $_POST['email']; //收货人邮箱
		$order['best_time']    = $_POST['best_time']; //收货人的最佳送货时间
		$order['sign_building']= $_POST['sign_building']; //收货人的地址的标志性建筑

		$order['postscript']  = "寄存发货订单"; //订单附言

		//是否是寄存发货的订单，如果是存储寄存ID
		$order['is_deposit'] = $deposit_id;


		$order['goods_amount']    = 0; //商品总金额
		...
		$new_order_id = $db->insert_id();//获得新生成的订单ID
		/* 增加寄存发货订单 */
		

		$arrGid = $_POST['numbethidd'];
		$arrNum = $_POST['number'];
		$test = "";
		foreach ($arrGid AS $key => $val)
		{
			...
			{
				/* 获得该产品的寄存数量 */
				$goods_name=""; $goods_sn=""; $goods_number=0;
				$sql = "SELECT goods_name, goods_sn, goods_number " .
					" FROM " . $ecs->table('deposit_goods') . 
					" WHERE dep_id = ".$deposit_id." AND goods_id = ".$val;
				...
				/* 添加商品到寄存订单 */
				if ($arrNum[$key] > $goods_number)
				{
					$arrNum[$key] = $goods_number;
				}

				$sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
					"order_id, goods_id, goods_name, goods_sn, goods_number) ".
					" values(". $new_order_id .",". $val .",'". $goods_name ."','". $goods_sn ."',". $arrNum[$key] .")";
				$db->query($sql);
				/* 添加商品到寄存订单 */


				/* 寄存商品数量减少，如果为0时删除该商品 */
				if (($goods_number-$arrNum[$key]) <= 0)
				{$test="DELETE";
					$sql = "DELETE from ".$ecs->table('deposit_goods'). " WHERE dep_id = ". $deposit_id ." AND goods_id = ". $val;
					$db->query($sql);
				}
				else
				{$test="UPDATE";
					$synum = $goods_number-$arrNum[$key];
				
					$sql = "UPDATE ".$ecs->table('deposit_goods'). " SET goods_number=". $synum . 
						" WHERE dep_id = ". $deposit_id ." AND goods_id = ". $val;
					$db->query($sql);
				}
				/* 寄存商品数量减少，如果为0时删除该商品 */
		...
		show_message('发货成功。发货订单号为：'. $order['order_sn'], '',  'user.php?act=deposit_list', 'info', false);

		//$smarty->assign('user_count', $test . " 发货操作   订单号=" .$order['order_sn'] . "  订单ID=" . $new_order_id);
		//$smarty->display('user_transaction.dwt');
	}
}
...
elseif ($action == 'order_detail')
{
    ...
    foreach ($goods_list AS $key => $value)
    {
		...
		if (intval($value['suit_id']) == 0)
		{
			$goods_list[$key]['goods_price']  = price_format($value['goods_price'], false);
			$goods_list[$key]['subtotal']     = price_format($value['subtotal'], false);
		}
		elseif (intval($value['suit_id']) > 0)
		{
			$goods_list[$key]['goods_price']  = '套餐内商品';
			$goods_list[$key]['subtotal']     = '套餐内商品';
		}
    }
...
elseif ($action == 'address_list')
{
    ...
    foreach ($consignee_list AS $region_id => $consignee)
    {
        ...
        $town_list[$region_id]     = get_regions(4, $consignee['district']);
    }
    ...
    $smarty->assign('town_list',    $town_list);
